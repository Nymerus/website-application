...
NymerusController.controller('NymerusUserFileCtrl', ['$scope', '$mdDialog', 'msgBus',
  ...
  function ($scope, $mdDialog, msgBus, socket, HTMLProvider) {
     ...
     */
    msgBus.onMsg('updatedShortcuts.U-F-C', function (event, data) {
      if (data !== null) {
        shortcuts = data;
     ...
     */
    msgBus.onMsg('advancedShortcuts.U-F-C', function (event, data) {
      if (data !== null) {
        shortcuts = data;
     ...
     */
    msgBus.onMsg('actionedShortcuts.U-F-C', function (event, data) {
      if (data !== null) {
        shortcuts = data;
        execAction();
      }
    ...
    function DialogController($scope, $mdDialog, item, contacts, isHost) {
      $scope.init = function () {
        console.log(item, contacts, isHost);
        $scope.members = [];
        $scope.owner = isHost;
        loadMembers(item);
